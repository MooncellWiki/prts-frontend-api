...
async def fetch_table(path):
    async with httpx.AsyncClient() as client:
        data = await client.get(
            f"https://raw.fastgit.org/Kengxxiao/ArknightsGameData/master/zh_CN/gamedata/{path}"
        )
...
async def get_skin_info():
    ...
    for skin_id, skin in skin_table["charSkins"].items():
        if brand_list.get(skin["displaySkin"]["skinGroupId"], False):
            ...
            skin_info[skin["charId"]][skin["skinId"]]["brand"] = brand_list[
                skin["displaySkin"]["skinGroupId"]
            ]
            skin_info[skin["charId"]][skin["skinId"]]["get_time"] = skin["displaySkin"][
                "getTime"
            ]
            skin_info[skin["charId"]][skin["skinId"]]["approach"] = skin["displaySkin"][
                "obtainApproach"
            ]
